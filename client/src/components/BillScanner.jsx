...
export default function BillScanner({ onScanSuccess }) {
  ...
  const handleScan = useCallback(async () => {
    ...
    try {
      ...
      if (err.message.includes('overloaded') || err.message.includes('429')) {
        ...
        userFriendlyMsg =
          'Unable to connect to the scanning server. Please verify the backend service is running.';
      }
  ...
  return (
    <div className="w-full bg-slate-900/40 border border-slate-800/80 rounded-2xl p-6 md:p-8 backdrop-blur-sm shadow-xl flex flex-col">
      ...
      <h3 className="text-xl font-bold font-display text-white mb-2">AI Bill Scanner</h3>
      <p className="text-slate-400 text-sm mb-6">
        Upload an image of your electricity or fuel bill to automatically scan metrics using Google
        Gemini.
      </p>
      ...
      {!previewUrl ? (
        ...
        >
          ...
          <p className="text-xs text-slate-400 mt-2">Supports JPEG, PNG, or WEBP up to 5MB</p>
        </div>
      ) : (
        ...
        <div className="flex flex-col space-y-4">
          <div className="relative rounded-xl overflow-hidden border border-slate-800 max-h-48 bg-slate-950 flex items-center justify-center">
            ...
            {isLoading && (
              <div
                className="absolute inset-0 bg-slate-950/70 backdrop-blur-xs flex items-center justify-center"
                aria-hidden="true"
              >
                <svg
      ...
      {error && (
        <div className="mt-4 p-3.5 bg-red-500/10 border border-red-500/20 rounded-xl flex items-start space-x-2">
          ...
          <span className="text-red-400 text-xs font-semibold leading-relaxed">{error}</span>
        </div>
      )}
    </div>
  );
